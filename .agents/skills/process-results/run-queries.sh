...
# Extract individual SQL statements from a multi-statement .sql file
# Returns them as numbered blocks: each block is a SQL statement
extract_statements() {
  local file="$1"
  local stmt=""
  local idx=0
  while IFS= read -r line; do
    # Skip empty lines — they may delimit statements in some files
    if [ -z "$line" ] && [ -n "$stmt" ]; then
      echo "STMT_${idx}:"
      echo "$stmt"
      stmt=""
      idx=$((idx + 1))
    ...
    if [ -n "$line" ]; then
      if [ -n "$stmt" ]; then stmt="$stmt $line"; else stmt="$line"; fi
    fi
  done < "$file"
...
run_queries_for_state() {
  ...
  # Statement 0: By party
  psql_csv \
    "SELECT PARTY, COUNT(*) AS NO_OF_CANDIDATES FROM $TABLE WHERE STATE = '$display' AND CANDIDATE <> 'NOTA' GROUP BY PARTY ORDER BY NO_OF_CANDIDATES DESC" \
    "$dir/01 Number Of Candidates.csv"

  # Statement 1: Total votes
  psql_csv \
    "SELECT SUM(VOTES) AS NO_OF_VOTES FROM $TABLE WHERE STATE = '$display'" \
    "$dir/01 Number Of Votes.csv"

  # Statement 2: Total candidates
  psql_csv \
    "SELECT COUNT(*) AS NO_OF_CANDIDATES FROM $TABLE WHERE STATE = '$display' AND CANDIDATE <> 'NOTA'" \
    "$dir/01 Number Of Candidates by State.csv"

  # ── 02 Maximum Votes for a Candidate ────────────────────────────────
  psql_csv \
    "SELECT CODE AS CONSTITUENCY_CODE, CANDIDATE, PARTY, CONSTITUENCY, STATE, VOTES FROM $TABLE WHERE STATE = '$display' ORDER BY VOTES DESC" \
    "$dir/02 Maximum Votes for a Candidate.csv"

  # ── 02b Max votes of winning candidates
  psql_csv \
    "SELECT t.CODE AS CONSTITUENCY_CODE, t.CANDIDATE, t.PARTY, t.CONSTITUENCY, t.STATE, t.VOTES AS WINNER_VOTES FROM $TABLE t JOIN (SELECT CODE, CONSTITUENCY, MAX(VOTES) AS MAX_VOTES FROM $TABLE WHERE STATE = '$display' GROUP BY CODE, CONSTITUENCY) sub ON t.CODE = sub.CODE AND t.CONSTITUENCY = sub.CONSTITUENCY AND t.VOTES = sub.MAX_VOTES WHERE t.STATE = '$display' ORDER BY t.VOTES DESC" \
    "$dir/02 Max votes of winning candidates.csv"

  # ── 03 Least votes for a winning candidate
  psql_csv \
    "SELECT t.CODE AS CONSTITUENCY_CODE, t.CANDIDATE, t.PARTY, t.CONSTITUENCY, t.STATE, t.VOTES AS WINNER_VOTES FROM $TABLE t JOIN (SELECT CODE, CONSTITUENCY, MAX(VOTES) AS MAX_VOTES FROM $TABLE WHERE STATE = '$display' GROUP BY CODE, CONSTITUENCY) sub ON t.CODE = sub.CODE AND t.CONSTITUENCY = sub.CONSTITUENCY AND t.VOTES = sub.MAX_VOTES WHERE t.STATE = '$display' ORDER BY WINNER_VOTES ASC" \
    "$dir/03 Least votes for a winning candidate.csv"

  # ── 04 Max votes for a losing candidate
  psql_csv \
    "SELECT t2.CODE AS CONSTITUENCY_CODE, t2.CANDIDATE, t2.PARTY, t2.CONSTITUENCY, t2.STATE, t2.VOTES AS RUNNER_VOTES FROM $TABLE t2 JOIN (SELECT t.CODE, t.CONSTITUENCY, MAX(t.VOTES) AS RUNNER_VOTES FROM $TABLE t JOIN (SELECT CODE, CONSTITUENCY, MAX(VOTES) AS MAX_VOTES FROM $TABLE WHERE STATE = '$display' GROUP BY CODE, CONSTITUENCY) sub ON t.CODE = sub.CODE AND t.CONSTITUENCY = sub.CONSTITUENCY AND t.VOTES < sub.MAX_VOTES WHERE t.STATE = '$display' GROUP BY t.CODE, t.CONSTITUENCY) sub2 ON t2.CODE = sub2.CODE AND t2.CONSTITUENCY = sub2.CONSTITUENCY AND t2.VOTES = sub2.RUNNER_VOTES WHERE t2.STATE = '$display' ORDER BY RUNNER_VOTES DESC" \
    "$dir/04 Max votes for a losing candidate.csv"

  # ── 05 Candidates winning by Max/Min margin
  psql_csv \
    "SELECT c1.CODE AS CONSTITUENCY_CODE, c1.STATE, c1.CONSTITUENCY, c1.CANDIDATE AS WINNER, c1.PARTY AS WINNER_PARTY, c1.VOTES AS WINNER_VOTES, c2.CANDIDATE AS RUNNER_UP, c2.PARTY AS RUNNER_UP_PARTY, c2.VOTES AS RUNNER_UP_VOTES, (c1.VOTES - c2.VOTES) AS VOTE_DIFFERENCE FROM (SELECT CODE, CONSTITUENCY, CANDIDATE, PARTY, VOTES, ROW_NUMBER() OVER (PARTITION BY CODE ORDER BY VOTES DESC) AS rn FROM $TABLE WHERE STATE = '$display') c1 JOIN (SELECT CODE, CONSTITUENCY, CANDIDATE, PARTY, VOTES, ROW_NUMBER() OVER (PARTITION BY CODE ORDER BY VOTES DESC) AS rn FROM $TABLE WHERE STATE = '$display') c2 ON c1.CODE = c2.CODE AND c1.CONSTITUENCY = c2.CONSTITUENCY AND c1.rn = 1 AND c2.rn = 2 ORDER BY VOTE_DIFFERENCE ASC" \
    "$dir/05 Candidates winning by Max or Min margin.csv"

  # ── 06 Max Total Votes in Constituency
  psql_csv \
    "SELECT CODE AS CONSTITUENCY_CODE, CONSTITUENCY, STATE, SUM(VOTES) AS TOTAL_VOTES FROM $TABLE WHERE STATE = '$display' GROUP BY CODE, CONSTITUENCY ORDER BY TOTAL_VOTES DESC" \
    "$dir/06 Max Total Votes in Constituency.csv"

  # ── 07 Max Candidates in a Constituency
  psql_csv \
    "SELECT CODE AS CONSTITUENCY_CODE, CONSTITUENCY, STATE, COUNT(CANDIDATE) AS NO_OF_CANDIDATES FROM $TABLE WHERE STATE = '$display' GROUP BY CODE, CONSTITUENCY ORDER BY NO_OF_CANDIDATES DESC" \
    "$dir/07 Max Candidates in a Constituency.csv"

  # ── 08 Total Vote Share
  psql_csv \
    "SELECT PARTY_VOTE_DETAILS.PARTY, PARTY_VOTE_DETAILS.PARTY_VOTES, (PARTY_VOTE_DETAILS.PARTY_VOTES::numeric / NULLIF(TOTAL_VOTE_DETAILS.TOTAL_VOTES, 0) * 100) AS VOTE_SHARE FROM (SELECT PARTY, SUM(VOTES) AS PARTY_VOTES FROM $TABLE WHERE STATE = '$display' GROUP BY PARTY) AS PARTY_VOTE_DETAILS, (SELECT SUM(VOTES) AS TOTAL_VOTES FROM $TABLE WHERE STATE = '$display') AS TOTAL_VOTE_DETAILS ORDER BY PARTY_VOTES DESC" \
    "$dir/08 Total Vote Share.csv"

  # ── 09 Least Vote share of Winning Candidate
  psql_csv \
    "SELECT v.CANDIDATE, v.PARTY, v.CONSTITUENCY, v.VOTE_SHARE FROM (SELECT ger.CANDIDATE, ger.PARTY, ger.CONSTITUENCY, ger.VOTES, ger.CODE, VOTE_DETAILS.TOTAL_VOTES, (ger.VOTES::numeric / NULLIF(VOTE_DETAILS.TOTAL_VOTES, 0) * 100) AS VOTE_SHARE FROM (SELECT CODE, CONSTITUENCY, SUM(VOTES) AS TOTAL_VOTES FROM $TABLE WHERE STATE = '$display' GROUP BY CODE, CONSTITUENCY) AS VOTE_DETAILS JOIN $TABLE ger ON VOTE_DETAILS.CONSTITUENCY = ger.CONSTITUENCY AND VOTE_DETAILS.CODE = ger.CODE WHERE ger.STATE = '$display') AS v JOIN (SELECT CODE, CONSTITUENCY, MAX(VOTES) AS MAX_VOTES FROM $TABLE WHERE STATE = '$display' GROUP BY CODE, CONSTITUENCY) AS WINNERS ON v.CONSTITUENCY = WINNERS.CONSTITUENCY AND v.CODE = WINNERS.CODE AND v.VOTES = WINNERS.MAX_VOTES ORDER BY VOTE_SHARE ASC" \
    "$dir/09 Least Vote share of Winning Candidate.csv"

  # ── 10 Max Vote share of a losing candidate
  psql_csv \
    "SELECT t2.CANDIDATE, t2.PARTY, t2.CONSTITUENCY, t2.VOTE_PERCENTAGE AS RUNNER_UP_VOTE_PERCENTAGE FROM $TABLE t2 JOIN (SELECT t.CODE, t.CONSTITUENCY, MAX(t.VOTES) AS RUNNER_VOTES FROM $TABLE t JOIN (SELECT CODE, CONSTITUENCY, MAX(VOTES) AS MAX_VOTES FROM $TABLE WHERE STATE = '$display' GROUP BY CODE, CONSTITUENCY) sub ON t.CODE = sub.CODE AND t.CONSTITUENCY = sub.CONSTITUENCY AND t.VOTES < sub.MAX_VOTES WHERE t.STATE = '$display' GROUP BY t.CODE, t.CONSTITUENCY) sub2 ON t2.CODE = sub2.CODE AND t2.CONSTITUENCY = sub2.CONSTITUENCY AND t2.VOTES = sub2.RUNNER_VOTES WHERE t2.STATE = '$display' ORDER BY RUNNER_UP_VOTE_PERCENTAGE DESC" \
    "$dir/10 Max Vote share of a losing candidate.csv"

  # ── 11 LOST Deposits
  psql_csv \
    "SELECT DEPOSIT_LOSERS.PARTY, COUNT(DEPOSIT_LOSERS.PARTY) AS NO_OF_CANDIDATES_WITH_LESS_THAN_ONE_SIXTH_VOTES FROM (SELECT ger.CANDIDATE, ger.PARTY, ger.CONSTITUENCY, ger.VOTES, VOTE_DETAILS.TOTAL_VOTES, (ger.VOTES::numeric / NULLIF(VOTE_DETAILS.TOTAL_VOTES, 0)) AS VOTE_SHARE FROM (SELECT CODE, CONSTITUENCY, SUM(VOTES) AS TOTAL_VOTES FROM $TABLE WHERE STATE = '$display' GROUP BY CODE, CONSTITUENCY) AS VOTE_DETAILS JOIN $TABLE ger ON VOTE_DETAILS.CONSTITUENCY = ger.CONSTITUENCY AND VOTE_DETAILS.CODE = ger.CODE WHERE ger.STATE = '$display') DEPOSIT_LOSERS WHERE VOTE_SHARE < (1.0 / 6) AND PARTY <> 'NONE OF THE ABOVE' GROUP BY DEPOSIT_LOSERS.PARTY ORDER BY NO_OF_CANDIDATES_WITH_LESS_THAN_ONE_SIXTH_VOTES DESC" \
    "$dir/11 LOST Deposits.csv"

  # ── 12 All Parties
  psql_csv \
    "SELECT DISTINCT PARTY FROM $TABLE WHERE STATE = '$display' AND PARTY NOT IN ('INDEPENDENT', 'NOTA') ORDER BY PARTY" \
    "$dir/12 All Parties.csv"

  # ── 12 Gold
  psql_csv \
    "SELECT PARTY, COUNT(*) AS SEATS_WON FROM (SELECT CONSTITUENCY, PARTY, VOTES, ROW_NUMBER() OVER (PARTITION BY CONSTITUENCY ORDER BY VOTES DESC) AS rn FROM $TABLE WHERE STATE = '$display') AS ranked_results WHERE rn = 1 GROUP BY PARTY ORDER BY SEATS_WON DESC" \
    "$dir/12 Gold.csv"

  # ── 13 Silver
  psql_csv \
    "SELECT PARTY, COUNT(*) AS SILVER_MEDALS FROM (SELECT CONSTITUENCY, PARTY, VOTES, ROW_NUMBER() OVER (PARTITION BY CONSTITUENCY ORDER BY VOTES DESC) AS rn FROM $TABLE WHERE STATE = '$display') AS ranked_results WHERE rn = 2 GROUP BY PARTY ORDER BY SILVER_MEDALS DESC" \
    "$dir/13 Silver.csv"

  # ── 14 Cost per vote
  psql_csv \
    "SELECT PARTY, TOTAL_COST, PARTY_VOTES, (TOTAL_COST::numeric / NULLIF(PARTY_VOTES, 0)) AS COST_PER_VOTE FROM (SELECT (COUNT(CANDIDATE) * 4000000) AS TOTAL_COST, GER.PARTY AS PARTY, PARTY_VOTE_DETAILS.PARTY_VOTES AS PARTY_VOTES FROM $TABLE GER JOIN (SELECT PARTY, SUM(VOTES) AS PARTY_VOTES FROM $TABLE WHERE STATE = '$display' GROUP BY PARTY) AS PARTY_VOTE_DETAILS ON GER.PARTY = PARTY_VOTE_DETAILS.PARTY WHERE GER.STATE = '$display' GROUP BY GER.PARTY, PARTY_VOTE_DETAILS.PARTY_VOTES) COST_ANALYSIS ORDER BY COST_PER_VOTE ASC" \
    "$dir/14 Cost per vote.csv"

  # ── 14 Success Ratio
  psql_csv \
    "SELECT p.PARTY, p.SEATS_PARTICIPATED, w.SEATS_WON, (w.SEATS_WON::numeric / NULLIF(p.SEATS_PARTICIPATED, 0) * 100) AS SUCCESS_RATIO FROM (SELECT PARTY, COUNT(DISTINCT CONCAT(STATE, '_', CONSTITUENCY)) AS SEATS_PARTICIPATED FROM $TABLE WHERE STATE = '$display' GROUP BY PARTY) p LEFT JOIN (SELECT PARTY, COUNT(*) AS SEATS_WON FROM (SELECT CONSTITUENCY, PARTY, VOTES, ROW_NUMBER() OVER (PARTITION BY CONSTITUENCY ORDER BY VOTES DESC) AS rn FROM $TABLE WHERE STATE = '$display') ranked_results WHERE rn = 1 GROUP BY PARTY) w ON p.PARTY = w.PARTY ORDER BY SUCCESS_RATIO DESC, SEATS_PARTICIPATED DESC" \
