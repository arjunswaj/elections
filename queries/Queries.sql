...
CREATE INDEX idx_assembly_elections_may2026_candidate
  ON assembly_elections_may2026 (candidate);
...
SELECT setval(
  pg_get_serial_sequence('assembly_elections_may2026', 'id'),
  (SELECT COALESCE(MAX(id), 1) FROM assembly_elections_may2026)
);

ANALYZE assembly_elections_may2026;

SELECT * FROM assembly_elections_may2026 LIMIT 25;

SELECT COUNT(*) FROM assembly_elections_may2026;