...
WITH candidate_counts AS
  ...
  (SELECT w.state,
   ...
   FROM ranked w
   JOIN ranked r ON w.state = r.state
   AND w.code = r.code
...
FROM margins m
JOIN candidate_counts c ON c.state = m.state
AND c.code = m.code
AND c.constituency = m.constituency
JOIN others o ON o.state = m.state
AND o.code = m.code
AND o.constituency = m.constituency
