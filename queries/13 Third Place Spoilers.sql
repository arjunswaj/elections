...
WITH ranked AS (
    ...
    FROM ranked w
    JOIN ranked r ON r.state = w.state AND r.code = w.code AND r.constituency = w.constituency AND r.rn = 2
    WHERE w.rn = 1
)
SELECT t.state,
...
JOIN candidate_counts cc ON cc.state = t.state AND cc.code = t.code AND cc.constituency = t.constituency
JOIN close_margins cm ON cm.state = t.state AND cm.code = t.code AND cm.constituency = t.constituency
WHERE pt.contests >= 5
